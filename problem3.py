...
# Consider what issues might arrise with this function
root5 = math.pow(5,0.5)
FUNCTIONS = [add, sub, mul]
TERMINALS = ['(1+root5)/2,power x', '(1-root5)/2,power x', 1/root5, -2, -1, 0, 1, 2]

def target_func(x): # evolution's target
    return (pow((1+root5),x)-pow((1-root5),x))/(pow(2,x)*root5)
...
class GPTree:
    ...
    def compute_tree(self, x):
        if (self.data in FUNCTIONS):
            ...
            return x
        elif self.data == '(1+root5)/2,power x':
            return pow((1 + root5) / 2, x)
        elif self.data == '(1-root5)/2,power x':
            return pow((1 - root5) / 2, x)
        else:
            return self.data
